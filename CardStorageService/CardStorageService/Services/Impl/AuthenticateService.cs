...
namespace CardStorageService.Services.Impl
{
    public class AuthenticateService : IAuthenticateService
    {
        ...
        public const string SecretKey = "LfsdfFSDFl23sdkjfKlfs==";

        private readonly Dictionary<string, SessionInfo> _sessions = new Dictionary<string, SessionInfo>();
        //коллекция сессий, ключ - токен, значение - информация о сессии
        ...
        {
            using IServiceScope scope = _serviceScopeFactory.CreateScope();//обращаемся к области пространства scope
            CardStorageServiceDbContext context = scope.ServiceProvider.GetRequiredService<CardStorageServiceDbContext>();
            ...
            if (account == null) // если пользователь не найден
            {
            ...
            if (!PasswordUtils.VerifyPassword(authenticationRequest.Password, account.PasswordSalt, account.PasswordHash)) //Если мы не прошли верификацию, то возвращаем response
            {
                return new AuthenticationResponse
                {
            ...
            lock (_sessions)//примитивная синхронизация
            {
                _sessions[sessionInfo.SessionToken] = sessionInfo;
            }
